...
**Standard:** Command names **MUST** follow `category.command-name` pattern

**Rules:**
- Kebab-case (lowercase with hyphens only)
- Category prefix required (e.g., `git`, `code`, `docs`)
- Separator is dot (`.`) between category and name
...
**Invalid Examples:**
```
✗ CodeReview              (no category prefix, not kebab-case)
✗ code_review             (no category, underscore instead of hyphen)
✗ git-code-review         (hyphen instead of dot separator)
✗ code.Review             (uppercase letter)
✗ review                  (no category prefix)
✗ code.this-is-a-very-long-command-name-that-exceeds-limits  (> 40 chars)
```
